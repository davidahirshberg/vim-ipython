...
def update_subchannel_msgs(debug=False, force=False, force_open=False):
    """
...
"""
save and restore restore pre-command search history, cursor position
based on code from http://technotales.wordpress.com/2010/03/31/preserve-a-vim-function-that-keeps-your-state/ 
"""
def vimexec(lines):
    for line in lines.strip().split('\n'):
        vim.command(line.strip())
...
def restoring_editor_state(func):
    import functools
...
@restoring_editor_state
@with_subchannel
def run_these_lines():
    _run_these_lines()

def _run_these_lines():
    r = vim.current.range
   
    # if we're running a single line, use specialized code
    if r.start == r.end: 
        run_this_line()
        return
...
def dedent_run_these_lines():
    ...
    if count > 0:
       vim.command("'<,'>" + "<"*count)
    with_subchannel(_run_these_lines)()
    if count > 0:
       vim.command("silent undo")
...
if g:ipy_perform_mappings != 0
    let mapleader = ","
    map <silent> <leader>f :py run_this_file()<CR>
    map <silent> <leader>d :py get_doc_buffer()<CR>
    map <silent> <leader>s :py if update_subchannel_msgs(force=True, force_open=True): echo("vim-ipython shell updated",'Operator')<CR>
    map <silent>  <leader>/  :py newline()<CR> 
    map <silent>  <leader>,  :py run_these_lines()<CR>
    map <silent>  <leader>.  :py dedent_run_these_lines()<CR>
    vmap <silent>  <leader>, :py run_these_lines()<CR>
    vmap <silent>  <leader>. :py dedent_run_these_lines()<CR>
    ...
    "map <silent> <leader>/ :python dedent_run_this_line()<CR>
